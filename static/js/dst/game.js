...
requestAnimationFrame(AC_GAME_ANIMATION);class GameMap extends AcGameObject {
    ...
    reset_fireball_coldtime() {
        this.fireball_coldtime = 3;
    }

    reset_blink_coldtime() {
        this.blink_coldtime = 5;
    }
    ...
    render_fireball_coldtime() {
        ...
        if (this.fireball_coldtime > 0) {
            ...
            let ratio = this.fireball_coldtime / 3;
            this.ctx.arc(x * scale, y * scale, r * scale, 0 - Math.PI / 2, Math.PI * 2 * (1 - ratio) - Math.PI / 2, true);
    ...
    render_blink_coldtime() {
        ...
        if (this.blink_coldtime > 0) {
            this.ctx.beginPath();
            this.ctx.moveTo(x * scale, y * scale);
            let ratio = this.blink_coldtime / 5;
            this.ctx.arc(x * scale, y * scale, r * scale, 0 - Math.PI / 2, Math.PI * 2 * (1 - ratio) - Math.PI / 2, true);
            this.ctx.lineTo(x * scale, y * scale);
            this.ctx.fillStyle = "rgba(0, 0, 255, 0.6)";
            this.ctx.fill();
    ...
    on_destroy() {
        ...
        for (let i = 0; i < this.playground.players.length; i++) {
            if (this.playground.players[0] === this) {
...
class MultiPlayerSocket {
    constructor(playground) {
        ...
        this.ws = new WebSocket("wss://app2243.acapp.acwing.com.cn/wss/multiplayer/");
...
<div class="ac-game-settings">
    <div class="ac-game-settings-login">
        ...
        <div class="ac-game-settings-acwing">
            <img width="30" src="https://app2243.acapp.acwing.com.cn/static/image/settings/acwing_logo.png">
            <br>
            <div>一键登录</div>
        </div>
    </div>
    <div class="ac-game-settings-register">
        ...
        this.$login_error_messages = this.$login.find(".ac-game-settings-error-message");
        ...
        this.$register_error_messages = this.$register.find(".ac-game-settings-error-message");
    ...
    login_on_remote() {
        ...
        $.ajax({
            url: "https://app2243.acapp.acwing.com.cn/settings/login/",
            type: "GET",
            data: {
                username: username,
                password: password,
    ...
    register_on_remote() {
        ...
        $.ajax({
            url: "https://app2243.acapp.acwing.com.cn/settings/register/",
            type: "GET",
            data: {
                username: username,
                password: password,
    ...
    getinfo_acapp() {
        let outer = this;
        $.ajax({
            url: "https://app2243.acapp.acwing.com.cn/settings/acwing/acapp/apply_code/",
            type: "GET",
            success: function(resp) {
                if (resp.result === "success") {
